...
type SortKey = "name" | "status" | "amount" | "dueDate" | null
type SortDir = "asc" | "desc"

const allCharges = [
  { id: "COB-001", name: "Maria Silva", status: "paid", amount: 150000, dueDate: "2024-02-15" },
  { id: "COB-002", name: "João Santos", status: "pending", amount: 75000, dueDate: "2024-02-20" },
  { id: "COB-003", name: "Ana Costa", status: "overdue", amount: 32000, dueDate: "2024-01-10" },
  { id: "COB-004", name: "Carlos Oliveira", status: "paid", amount: 89900, dueDate: "2024-02-18" },
  { id: "COB-005", name: "Beatriz Lima", status: "pending", amount: 45000, dueDate: "2024-03-01" },
  { id: "COB-006", name: "Pedro Souza", status: "cancelled", amount: 120000, dueDate: "2024-01-25" },
  { id: "COB-007", name: "Fernanda Alves", status: "paid", amount: 67500, dueDate: "2024-02-28" },
  { id: "COB-008", name: "Lucas Pereira", status: "overdue", amount: 23000, dueDate: "2024-01-05" },
]
...
export default function ListPageDemo() {
  ...
  const [visibleCols, setVisibleCols] = useState({ name: true, status: true, amount: true, dueDate: true })
  const perPage = 5

  // Filter
  let filtered = allCharges.filter((row) => {
      ...
      <div className="flex items-center justify-between">
        <div className="space-y-1">
          <nav className="flex text-sm text-muted-foreground">
            ...
            <span className="text-foreground">Cobranças</span>
          </nav>
          <h2 className="text-lg font-semibold">Cobranças</h2>
          <p className="text-sm text-muted-foreground">Gerencie suas cobranças</p>
        </div>
        <button className="inline-flex h-9 items-center gap-2 rounded-md bg-primary px-4 text-sm font-medium text-primary-foreground shadow-sm hover:bg-primary/90">
            ...
            <table className="w-full text-sm">
              <thead className="[&_tr]:border-b">
                <tr className="border-b transition-colors hover:bg-muted/50">
                  ...
                  {visibleCols.amount && <th className="h-12 px-4 text-right font-medium text-muted-foreground"><SortButton field="amount" label="Valor" /></th>}
                  {visibleCols.dueDate && <th className="h-12 px-4 text-left font-medium text-muted-foreground"><SortButton field="dueDate" label="Vencimento" /></th>}
                        ...
                        <div className="absolute right-0 top-full z-50 mt-1 w-[160px] rounded-md border bg-popover p-1 shadow-md">
                          {(Object.keys(visibleCols) as (keyof typeof visibleCols)[]).map((col) => (
                            ...
                            >
                              ...
                              {{ name: "Cliente", status: "Status", amount: "Valor", dueDate: "Vencimento" }[col]}
                            </button>
                          ))}
                        </div>
                      )}
              ...
              <tbody className="[&_tr:last-child]:border-0">
                {paged.length === 0 ? (
                  <tr><td colSpan={6} className="h-24 text-center text-muted-foreground">Nenhuma cobrança encontrada.</td></tr>
                ) : (
                  paged.map((row) => (
                    <tr key={row.id} className="group border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted" data-state={selected.has(row.id) ? "selected" : undefined}>
                      ...
                      {visibleCols.dueDate && <td className="p-4 text-muted-foreground">{row.dueDate}</td>}
